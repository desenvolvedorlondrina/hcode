    ...
    IF pid_cart > 0 THEN
        UPDATE tb_carts
           SET des_session_id = pdes_session_id,
               ...
               num_days = pnum_days,
               is_own_delivery = pis_own_delivery
         WHERE id_cart = pid_cart;

    ELSE
        INSERT INTO tb_carts (des_session_id, id_user, id_address, num_temporary_zip_code, vl_freight, des_type_freight, num_days, is_own_delivery, dt_cart_created_at)
                      VALUES (pdes_session_id, pid_user, pid_address, pnum_temporary_zip_code, pvl_freight, pdes_type_freight, pnum_days, pis_own_delivery, NOW());
