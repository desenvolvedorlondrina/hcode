...
class Cart extends Model implements JsonSerializable
{
     ...
     * Propriedade
     *
     * @var array<string,string> Colunas de mapeamento objeto relacional
     */
    private static array $_columns = [
        ...
        "ownDelivery" => "is_own_delivery", // O envio é próprio
    ...
    public function save(): self
    {
        $query = "CALL sp_save_cart (:pid_cart, :pdes_session_id, :pid_user, :pid_address, :pnum_temporary_zip_code,
                                     :pvl_freight, :pdes_type_freight, :pnum_days, :pis_own_delivery)";
        ...
        $stmt->bindValue("pis_own_delivery", $this->ownDelivery, \PDO::PARAM_BOOL);
        $stmt->execute();
    ...
    public function getProducts(bool $related = true): array
    {
        if ($related) {
            $query = "SELECT p.*
                        FROM tb_products p
                       INNER JOIN tb_carts_products pc USING (id_product)
    ...
    public static function loadFromData(array $arguments, ?self $cart = null): self
    {
        ...
        $cart->ownDelivery = (bool) ($arguments["ownDelivery"] ?? false);
    ...
    private function _calculateFreight(): self
    {
        if ($this->_validateZipCode((string) $this->temporaryZipCode) && $this->package->quantity > 0) {
            ...
            curl_setopt_array(
                $ch,
                [
                    \CURLOPT_URL => "https://shopping.correios.com.br/wbm/shopping/script/calcprecoprazo.asmx/CalcPrecoPrazo?$queryString",
            ...
            $this->days = $result?->PrazoEntrega ?? (float) getenv("COURIER_MAXIMUM_DELIVERY_TIME");
            $this->ownDelivery = empty($result?->MsgErro);
            $this->freightValue = $result?->Valor ? $this->_decimalToFloat($result?->Valor) : (float) getenv("COURIER_DEFAULT_SHIPPING_PRICE");
