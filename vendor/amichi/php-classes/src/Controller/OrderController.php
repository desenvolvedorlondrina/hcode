...
class OrderController extends Controller
{
    ...
    public static function getByUser(Request $request, Response $response, array $args): Response
    {
        ...
        if ($sessionUser->id !== $idUser && !$sessionUser->isAdmin) {
            throw new HttpException("Não foi possível consultar os pedidos do usuário $idUser, pois, você não possui permissão.", 400);
        }
    ...
    public static function getBankPaymentSlip(Request $request, Response $response, array $args): Response
    {
        ...
        if ($sessionUser->id !== $order->idUser && !$sessionUser->isAdmin) {
            throw new HttpException("Não foi possível consultar o boleto do pedido $id, pois, você não possui permissão.", 400);
        }
    ...
    public static function post(Request $request, Response $response, array $args): Response
    {
        ...
        if ($errors) {
            ...
            throw new HttpException("Não foi possível cadastrar o pedido. $message: " . implode(", ", $errors) . ".", 400);
        }
    ...
    public static function put(Request $request, Response $response, array $args): Response
    {
        ...
        if (!Order::loadFromId($id)) {
            throw new HttpException("Não foi possível alterar o pedido $id, pois, é inexistente.", 400);
        }
        ...
        if ($errors) {
            $message = count($errors) === 1 ? "O seguinte erro foi encontrado" : "Os seguintes erros foram encontrados";
            throw new HttpException("Não foi possível alterar o pedido $id. $message: " . implode(", ", $errors) . ".", 400);
        }
    ...
    public static function delete(Request $request, Response $response, array $args): Response
    {
        ...
        if (!$order) {
            throw new HttpException("Não foi possível remover o pedido $id, pois, é inexistente.", 400);
        }
