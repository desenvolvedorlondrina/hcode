...
class OtherController extends Controller
{
    ...
    public static function forgot(Request $request, Response $response, array $args): Response
    {
        ...
        if (!$user) {
            throw new HttpException("Não foi possível solicitar a redefinição de senha, pois, o endereço de e-mail é inexistente.", 400);
        }
    ...
    public static function resetPassword(Request $request, Response $response, array $args): Response
    {
        ...
        if (!$userPasswordRecovery || $errors) {
            ...
            throw new HttpException("Não foi possível restaurar a senha do usuário. $message: " . implode(", ", $errors) . ".", 400);
        }
        ...
        if ($errors) {
            $message = count($errors) === 1 ? "O seguinte erro foi encontrado" : "Os seguintes erros foram encontrados";
            throw new HttpException("Não foi possível restaurar a senha do usuário {$userPasswordRecovery->idUser}. $message: " . implode(", ", $errors) . ".", 400);
        }
    ...
    public static function changePassword(Request $request, Response $response, array $args): Response
    {
        ...
        if (!$user) {
            throw new HttpException("Não foi possível alterar a senha do usuário, pois, não está autenticado.", 400);
        }
        ...
        if (!password_verify($oldPassword, $user->password)) {
            throw new HttpException("Não foi possível alterar a senha do usuário, pois, a senha atual está incorreta.", 400);
        }

        if (password_verify($newPassword, $user->password)) {
            throw new HttpException("Não foi possível alterar a senha do usuário, pois, a nova senha é igual a senha anterior.", 400);
        }
        ...
        if ($errors) {
            $message = count($errors) === 1 ? "O seguinte erro foi encontrado" : "Os seguintes erros foram encontrados";
            throw new HttpException("Não foi possível alterar a senha do usuário {$user->id}. $message: " . implode(", ", $errors) . ".", 400);
        }
    ...
    public static function zipCode(Request $request, Response $response, array $args): Response
    {
        ...
        if ($status !== 200 || isset($data["erro"])) {
            throw new HttpException("Não foi possível encontrar o CEP \"$zipCode\".", 400);
        }
