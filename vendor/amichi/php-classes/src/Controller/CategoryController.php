...
class CategoryController extends Controller
{
    ...
    public static function post(Request $request, Response $response, array $args): Response
    {
        ...
        if ($errors) {
            ...
            throw new HttpException("Não foi possível cadastrar a categoria. $message: " . implode(", ", $errors) . ".", 400);
        }
    ...
    public static function put(Request $request, Response $response, array $args): Response
    {
        ...
        if (!Category::loadFromId($id)) {
            throw new HttpException("Não foi possível alterar a categoria $id, pois, é inexistente.", 400);
        }
        ...
        while ($superCategory = Category::loadFromId((int) $idSuper)) {
            if ($superCategory->id === $category->id) {
                throw new HttpException("Não foi possível alterar a categoria $id, pois, a categoria mãe entra em loop.", 400);
            }
        ...
        if ($errors) {
            $message = count($errors) === 1 ? "O seguinte erro foi encontrado" : "Os seguintes erros foram encontrados";
            throw new HttpException("Não foi possível alterar a categoria $id. $message: " . implode(", ", $errors) . ".", 400);
        }
    ...
    public static function delete(Request $request, Response $response, array $args): Response
    {
        ...
        if (!$category) {
            throw new HttpException("Não foi possível remover a categoria $id, pois, é inexistente.", 400);
        }
    ...
    public static function postProduct(Request $request, Response $response, array $args): Response
    {
        ...
        if (!$category) {
            throw new HttpException("Não foi possível cadastrar o produto $idProduct na categoria $idCategory, pois, a categoria é inexistente.", 400);
        }

        $product = Product::loadFromId($idProduct);
        if (!$product) {
            throw new HttpException("Não foi possível cadastrar o produto $idProduct na categoria $idCategory, pois, o produto é inexistente.", 400);
        }

        $products = $category->getProducts();
        if (in_array($idProduct, array_map(fn (Product $product): int => $product->id, $products))) {
            throw new HttpException("Não foi possível cadastrar o produto $idProduct na categoria $idCategory, pois, o produto já está relacionado.", 400);
        }
    ...
    public static function deleteProduct(Request $request, Response $response, array $args): Response
    {
        ...
        $category = Category::loadFromId($idCategory);
        if (!$category) {
            throw new HttpException("Não foi possível remover o produto $idProduct da categoria $idCategory, pois, a categoria é inexistente.", 400);
        }

        $product = Product::loadFromId($idProduct);
        if (!$product) {
            throw new HttpException("Não foi possível remover o produto $idProduct da categoria $idCategory, pois, o produto é inexistente.", 400);
        }

        $products = $category->getProducts(false);
        if (in_array($idProduct, array_map(fn (Product $product): int => $product->id, $products))) {
            throw new HttpException("Não foi possível remover o produto $idProduct na categoria $idCategory, pois, o produto não está relacionado.", 400);
        }
