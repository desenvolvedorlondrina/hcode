...
class CartController extends Controller
{
    ...
    public static function get(Request $request, Response $response, array $args): Response
    {
        ...
        if (($sessionCart?->id !== $id || $sessionCart?->idUser !== $sessionUser->id) && !$sessionUser->isAdmin) {
            throw new HttpException("Não foi possível consultar o carrinho $id, pois, você não possui permissão.", 400);
        }
    ...
    public static function getProducts(Request $request, Response $response, array $args): Response
    {
        ...
        $sessionUser = User::loadFromSession();
        if (($sessionCart?->id !== $id || $sessionCart?->idUser !== $sessionUser->id) && !$sessionUser->isAdmin) {
            throw new HttpException("Não foi possível consultar os produtos do carrinho $id, pois, você não possui permissão.", 400);
        }
    ...
    public static function post(Request $request, Response $response, array $args): Response
    {
        if (Cart::loadFromSessionId()) {
            throw new HttpException("Não foi possível cadastrar o carrinho, pois, há um carrinho armazenado na sessão.", 400);
        }
        ...
        if ($errors) {
            ...
            throw new HttpException("Não foi possível cadastrar o carrinho. $message: " . implode(", ", $errors) . ".", 400);
        }
    ...
    public static function postProduct(Request $request, Response $response, array $args): Response
    {
        ...
        if (($sessionCart?->id !== $idCart || $sessionCart?->idUser !== $sessionUser?->id) && (!$sessionUser?->isAdmin)) {
            throw new HttpException("Não foi possível inserir o produto $idProduct no carrinho $idCart, pois, você não possui permissão.", 400);
        }

        $cart = Cart::loadFromId($idCart);
        if (!$cart) {
            throw new HttpException("Não foi possível inserir o produto $idProduct no carrinho $idCart, pois, o carrinho é inexistente.", 400);
        }

        $product = Product::loadFromId($idProduct);
        if (!$product) {
            throw new HttpException("Não foi possível inserir o produto $idProduct no carrinho $idCart, pois, o produto é inexistente.", 400);
        }

        $order = Order::loadFromCartId($idCart);
        if ($order) {
            throw new HttpException("Não foi possível inserir o produto $idProduct no carrinho $idCart, pois, o carrinho se tornou um pedido.", 400);
        }
    ...
    public static function put(Request $request, Response $response, array $args): Response
    {
        ...
        if (!$cartDB) {
            throw new HttpException("Não foi possível alterar o carrinho $id, pois, é inexistente.", 400);
        }

        $sessionCart = Cart::loadFromSessionId();
        $sessionUser = User::loadFromSession();
        if (($sessionCart?->id !== $id || $sessionCart?->idUser !== $sessionUser->id) && (!$sessionUser->isAdmin)) {
            throw new HttpException("Não foi possível alterar o carrinho $id, pois, você não possui permissão.", 400);
        }
        ...
        if ($errors) {
            $message = count($errors) === 1 ? "O seguinte erro foi encontrado" : "Os seguintes erros foram encontrados";
            throw new HttpException("Não foi possível alterar o carrinho $id. $message: " . implode(", ", $errors) . ".", 400);
        }

        $order = Order::loadFromCartId($id);
        if ($order) {
            throw new HttpException("Não foi possível alterar o carrinho $id, pois, este se tornou um pedido.", 400);
        }
    ...
    public static function delete(Request $request, Response $response, array $args): Response
    {
        ...
        if (!$cart) {
            throw new HttpException("Não foi possível remover o carrinho $id, pois, é inexistente.", 400);
        }

        $order = Order::loadFromCartId($id);
        if ($order) {
            throw new HttpException("Não foi possível remover o carrinho $id, pois, está vinculado ao pedido {$order->id}.", 400);
        }
    ...
    public static function deleteProduct(Request $request, Response $response, array $args): Response
    {
        ...
        $sessionUser = User::loadFromSession();
        if (($sessionCart?->id !== $idCart || $sessionCart?->idUser !== $sessionUser?->id) && (!$sessionUser?->isAdmin)) {
            throw new HttpException("Não foi possível remover o produto $idProduct do carrinho $idCart, pois, você não possui permissão.", 400);
        }

        $cart = Cart::loadFromId($idCart);
        if (!$cart) {
            throw new HttpException("Não foi possível remover o produto $idProduct do carrinho $idCart, pois, o carrinho é inexistente.", 400);
        }

        if (!$cart->containsProduct($idProduct)) {
            throw new HttpException("Não foi possível remover o produto $idProduct do carrinho $idCart, pois, o produto não está relacionado.", 400);
        }

        $product = Product::loadFromId($idProduct);
        if (!$product) {
            throw new HttpException("Não foi possível remover o produto $idProduct do carrinho $idCart, pois, o produto é inexistente.", 400);
        }

        $order = Order::loadFromCartId($idCart);
        if ($order) {
            throw new HttpException("Não foi possível remover o produto $idProduct do carrinho $idCart, pois, o carrinho se tornou um pedido.", 400);
        }
