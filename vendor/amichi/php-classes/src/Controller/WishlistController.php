...
class WishlistController extends Controller
{
    ...
    public static function getByUser(Request $request, Response $response, array $args): Response
    {
        ...
        if ($sessionUser->id !== $idUser && !$sessionUser->isAdmin) {
            throw new HttpException("Não foi possível consultar os produtos da lista de desejos do usuário $idUser, pois, você não possui permissão.", 400);
        }
    ...
    public static function post(Request $request, Response $response, array $args): Response
    {
        ...
        $sessionUser = User::loadFromSession();
        if ($sessionUser->id !== $idUser && !$sessionUser->isAdmin) {
            throw new HttpException("Não foi possível adicionar o produto à lista de desejos do usuário $idUser, pois, você não possui permissão.", 400);
        }

        $wishlist = Wishlist::loadFromUserAndProductId($idUser, $idProduct);

        if ($wishlist) {
            throw new HttpException("Não foi possível adicionar o produto à lista de desejos, pois, já foi adicionado anteriormente.", 400);
        }
        ...
        if ($errors) {
            ...
            throw new HttpException("Não foi possível adicionar o produto à lista de desejos. $message: " . implode(", ", $errors) . ".", 400);
        }
    ...
    public static function delete(Request $request, Response $response, array $args): Response
    {
        ...
        $sessionUser = User::loadFromSession();
        if ($sessionUser->id !== $idUser && !$sessionUser->isAdmin) {
            throw new HttpException("Não foi possível remove o produto da lista de desejos do usuário $idUser, pois, você não possui permissão.", 400);
        }

        $wishlist = Wishlist::loadFromUserAndProductId($idUser, $idProduct);

        if (!$wishlist) {
            throw new HttpException("Não foi possível remover o produto da lista de desejos, pois, é inexistente.", 400);
        }
